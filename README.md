# it-项目管理
4.1登陆页面的实现
用户通过用户名称即账号，用户密码和身份进行登陆，同时校验是否查看服务协议和个人信息保护政策，是否输入用户名和密码以及输入信息是否正确。登陆成功后，管理员和客户分别进入不同的页面。
 <el-main>
              <!-- 我们开始写中间的部分 
...
  },
4.2注册账户的实现
点击免费注册，用步骤条形式注册账号，第一步：创建用户名，第二步：设置姓名、密码并对密码进行二次确认，设置联系方式及身份，第三步：注册成功，点击跳转登陆页面。	
<el-main>
            <!-- 加一个进度条 -->
...
4.3用户管理的实现
   管理员可管理客户注册的账号，后台会显示客户所填写的信息，管理员可以通过客户的用户名查询客户信息，同时也可以进行编辑和删除操作。
<div class="table">
      <el-card class="zhu">
...
      },
4.4房型添加的实现
  管理员可以增加向客户展示的房间信息，如房间类型、房间简介、退房要求、星级、房间价格、服务设施、封面。
...
4.5订单管理的实现
  客户通过点击预订按钮，生成对应房间的订单，订单可显示订单编号，预定时间，房间类型，退房规则，入住人等信息，客户可以点击我的订单查看该用户下的历史订单并可进行退房删除操作。同时管理员可收到所有客户的订单信息并进行查看和退房删除。
 <el-main>
        <div class="main">
            <el-card>
              <div slot="header" >
...
  },
4.6房间报修的实现
  客户可同过点击订单中的保修按钮对此房间进行报修，并进行故障描述，系统根据订单信息完善报修信息，存入表中。管理员可查看所有客户的报修内容，并通过订单号查找。管理员可以添加报修信息并进行编辑和删除。
